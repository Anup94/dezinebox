...
if(isset($_POST["e"])){
	...
	if($numrows > 0){
        ...
        include("PHPMailer/PHPMailer/PHPMailerAutoload.php");
            ...
            $msg='<h2>Hello '.$u.'</h2><p>This is an automated message from #SparkYourBestseller. If you did not recently initiate the Forgot Password process, kindly disregard this email.</p><p>You indicated that you have forgotten your password. We can generate a temporary password for you to log in with.You should change your password pronto.</p><p>Your temporary password is:<b>'.$tempPass.'</b></p><p><a target="_blank" href="http://sparkyourbestseller.com/next.php?u='.$u.'&e='.$e.'&p='.$hashTempPass.'">Click here to log in</a></p><p>If you do not click the link in this email, no changes will be made to your account.</p>';
            // $msg='<h2>Hello <b>'.$username.'</b></h2><p>Thank you for signing into <b>#SPARKYOURBESTSELLER.</b></p>'; // HTML message
            $subject="Reset Password ! #SparkYourBestseller";
...
if(isset($_GET['u']) && isset($_GET['p'])){
	...
	$numrows = mysqli_num_rows($query);
	if($numrows == 0){
		header("location: message.php?msg=There is no match for that Email ID with that temporary password in the system. We cannot proceed.");
    	exit();
	} 
	else {
		echo $temppasshash;
...
function forgotpass(){
  ...
  if(e == ""){
    ...
    <meta name="author" content="#Sparkyourbestseller">
        <title>
            Forgot Password | SparkYourBestseller
        </title>
        <style type="text/css">
                  .modal-style {
            font-size: 20px;
